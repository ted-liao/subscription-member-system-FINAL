...
function initCalculator() {
    database.ref('calculatorConfig').on('value', (snapshot) => {
        ...
        if (currentPage === 'calculator') {
            updateWeekDisplay();
            renderPricingTables();
            // 【修改】呼叫 calculate，不傳參數，預設為 false (不彈 Alert)
            calculate(false); 
        }
    });
}
...
function getSeasonWeek() {
    if (!calcConfig || !calcConfig.seasonStartDate) return 1;
    ...
    // 如果今天比開始日期早，就是 Week 1
    if (now < start) return 1;

    // 計算天數差 (使用 Math.round 避免日光節約時間或毫秒誤差)
    ...
    if (calcConfig.seasonEndDate) {
        ...
        const totalDays = Math.round(totalDiff / (1000 * 60 * 60 * 24));
...
function calculate(isManualClick = false) {
    ...
    // 2. 只有在按下「開始報價」按鈕 (isManualClick 為 true) 時，才執行警告判定
    if (isManualClick) {
        // 分數門檻檢查
        if (current < 4501) {
        ...
        // 服務上限檢查
        if (currentServiceType === 'carry' && target > 13000) {
            alert("⚠️ 護航服務上限為 13,000 分，超過此分數請直接私訊主播洽詢特殊報價。");
            return;
        }
        if (currentServiceType === 'boost' && target > 15000) {
            alert("⚠️ 代打服務上限為 15,000 分，超過此分數請直接私訊主播洽詢特殊報價。");
            return;
        }
    ...
    // 3. 計算與更新數據 (只有在結果區塊已經是「顯示」的狀態下才更新，避免背景跳動)
    if (document.getElementById('resultContent').style.display === 'block') {
        ...
        const TIERS = [
            ...
            { name: 'legend', label: '傳奇(10000-)', min: 8001, max: 10000 },
            { name: 'mythical', label: '萬分(10000+)', min: 10001, max: 99999 }
        ];
        ...
        document.getElementById('estTime').textContent = `約 ${Math.ceil((scoreDiff / efficiency) / 5)} 天`;
    }
}
// ==========================================
