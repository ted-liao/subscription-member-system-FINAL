        ...
        services.forEach(service => {
            ranks.forEach(rank => {
                ...
                <div class="cell-wrapper">
                    <div class="input-row-mini"><span class="input-label-mini">W</span>
                        <input type="number" step="0.05" class="cell-input" id="w_${service}_${i}_${rank}" value="${data.w || 0}" onkeyup="calcCellWage(this, '${service}', ${i}, '${rank}', ${basePrice})">
                    </div>
                    <div class="input-row-mini"><span class="input-label-mini">E</span>
                        <input type="number" class="cell-input input-efficiency" id="e_${service}_${i}_${rank}" value="${data.e || 0}" onkeyup="calcCellWage(this, '${service}', ${i}, '${rank}', ${basePrice})">
                    </div>
                    <div id="wage_${service}_${i}_${rank}" class="wage-display">$${wage}/hr</div>
                </div>
            </td>`;
        });
    });
...
// [修改] 更新狀態顯示
// 在 admin.js 中替換掉整個 updateSeasonStatusDisplay 函式
function updateSeasonStatusDisplay() {
    ...
    const textEl = document.getElementById('currentWeekText'); // 確保抓到對的 ID
    const rangeEl = document.getElementById('seasonDateRangeText');

    // 基本檢查
    if (!startInput || !endInput || !displayBox) return;

    displayBox.style.display = 'block';
    if (rangeEl) rangeEl.textContent = `${startInput} ~ ${endInput}`;
